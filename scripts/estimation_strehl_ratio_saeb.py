...
def run(yaml_file):
    wavelength_nm = 2200 # wavelength in nm
    ...
    total_var = total_variance(var_fit, var_temp_atmo_CL + var_vibr_CL, var_alias_CL, var_meas_CL)
    ...
    summary_result =  summary_display(var_fit_modes, var_temp_modes, var_alias_modes, var_meas_modes,
    ...
    total_var = summary_result['totals']['total']
    sr_estimation = np.exp(-(np.sqrt(total_var)*2*np.pi/wavelength_nm)**2)
    print()
    print(f"Total variance: {np.sqrt(total_var):.3f}")
    print(f"Estimated Strehl ratio: {sr_estimation:.3f}")
